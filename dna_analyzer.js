...
class DNAAnalyzer {
  ...
  parseDNA(filteredContent) {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 4) {
        const rsid = parts[0];
        const allele1 = parts[2];
        const allele2 = parts[3];
        const genotype = this.normalizeGenotype(allele1, allele2);
  ...
  generateShapeVariants() {
    ...
    categories.forEach(category => {
      ...
      // Return 0-7 for 8 shape variants per family
      shapeVariants[category] = hash % 8;
    });
