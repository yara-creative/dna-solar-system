...
async function handleFileUpload(file) {
    ...
    reader.onload = async (e) => {
        ...
        console.log('📄 First 3 lines:', fileContent.split('\n').slice(0, 3));
        ...
        console.log('Original SNPs:', originalCount);
        console.log('Filtered SNPs:', filteredCount);
        console.log('Filtered content preview:', filteredContent.split('\n').slice(0, 3));
        
        console.log('🎯 FILTERED CONTENT READY:');
        console.log('   Filtered SNPs:', filteredCount);
        console.log('   Filtered content preview (first 200 chars):', filteredContent.substring(0, 200));

        loadingBar.style.width = '30%';

        window.dnaAnalyzer = new DNAAnalyzer(window.knowledgeBase.KNOWLEDGE_BASE);
        
        console.log('📤 SENDING TO ANALYZER:', filteredContent.split('\n').length, 'lines');
        window.dnaAnalyzer.parseDNA(filteredContent);
        console.log('✅ ANALYZER FINISHED PARSING');
        
        console.log('🔬 MATCHED SNPs in analyzer:', window.dnaAnalyzer.matchedSnps ? window.dnaAnalyzer.matchedSnps.length : 'undefined');
        
        window.dnaAnalysis = window.dnaAnalyzer.generateFullAnalysis();

        console.log('=== STEP 4: STORE ANALYSIS ===');
        console.log('window.dnaAnalysis:', window.dnaAnalysis);
...
function startTransition() {
    ...
    const vibrantColors = [0xff6b9d, 0x9b59b6, 0x3498db, 0x1abc9c, 0xf39c12, 0xe74c3c, 0xc44569];

    const shapeVariants = window.dnaAnalyzer ? window.dnaAnalyzer.generateShapeVariants() : null;

    console.log('🧬 Generated Shape Variants:', shapeVariants);
    ...
    for (let i = 0; i < numObjects; i++) {
        ...
        if (shapeVariants && ShapeGenerator) {
            const category = categories[i];
            const shapeData = shapeVariants[category];
            console.log(`🔹 Planet ${i}: category="${category}", family=${shapeData.family}, variant=${shapeData.variant}`);
            const group = ShapeGenerator.createShape(shapeData.family, shapeData.variant, x, y, z, color, angle);
...
function updateTextPanel(objData) {
    ...
    const fullHTML = `
        <div style="margin-bottom: 20px;">
            ...
            <p style="color: #666; font-size: 12px; margin-bottom: 15px;">${category.subtitle}</p>
        </div>
        ${analysisHTML}
    `;
    
    document.getElementById('text-content').innerHTML = fullHTML;
}
